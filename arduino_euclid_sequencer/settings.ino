...
static const char *SETTING_PARAMETER_NAMES[] = {
  ...
  SETTING_PARAMETER_NAME_SLEN, // 1  - sequence length
  SETTING_PARAMETER_NAME_SPUL, // 2  - active sequence pulses - this is in the Euclidian Rythm sense
  SETTING_PARAMETER_NAME_SROT, // 3  - sequence offset - this is in the Euclidian Rythm sense
  SETTING_PARAMETER_NAME_SDIR, // 4  - sequence play direction - forward, backward, alternating
  SETTING_PARAMETER_NAME_SNLN, // 5  - sequence note length - 16th, 8th, 4th
  SETTING_PARAMETER_NAME_SGTE, // 6  - sequence gate length
  SETTING_PARAMETER_NAME_SVEL, // 7  - sequence note velocity
  SETTING_PARAMETER_NAME_SNTE, // 8  - sequence MIDI note
...
uint8_t setting_parameterValue_Snln[8] = {SETTING_PARAMETER_VALUE_SNLN_SIXTEENTH, SETTING_PARAMETER_VALUE_SNLN_SIXTEENTH, SETTING_PARAMETER_VALUE_SNLN_SIXTEENTH, SETTING_PARAMETER_VALUE_SNLN_SIXTEENTH, SETTING_PARAMETER_VALUE_SNLN_SIXTEENTH, SETTING_PARAMETER_VALUE_SNLN_SIXTEENTH, SETTING_PARAMETER_VALUE_SNLN_SIXTEENTH, SETTING_PARAMETER_VALUE_SNLN_SIXTEENTH};
uint8_t setting_parameterValue_Sgte[8] = {50, 50, 50, 50, 50, 50, 50, 50};
uint8_t setting_parameterValue_Svel[8] = {100, 100, 100, 100, 100, 100, 100, 100};
uint8_t setting_parameterValue_Snte[8] = {36, 38, 42, 46, 37, 75, 48, 41};
...
void setting_changeParameter(int32_t delta) {
  ...
  if (setting_parameterIndex == 0) {
    ...
    // SETTING_PARAMETER_NAME_SGTE - sequence gate length
    setting_parameterValue_tmp = setting_parameterValue_Sgte[seqId];
  } else if (setting_parameterIndex == 7) {
    // SETTING_PARAMETER_NAME_SVEL - sequence note velocity
    setting_parameterValue_tmp = setting_parameterValue_Svel[seqId];
  } else if (setting_parameterIndex == 8) {
    // SETTING_PARAMETER_NAME_SNTE - sequence MIDI note
    setting_parameterValue_tmp = setting_parameterValue_Snte[seqId] - SETTING_PARAMETER_VALUE_SNTE_MIN;
...
char* setting_getParameterValue() {
  ...
  if (setting_parameterIndex == 0) {
    ...
    // SETTING_PARAMETER_NAME_SGTE - sequence gate length
    sprintf(returnValue, "%3d", setting_parameterValue_Sgte[seqId]);
  } else if (setting_parameterIndex == 7) {
    // SETTING_PARAMETER_NAME_SVEL - sequence note velocity
    sprintf(returnValue, "%3d", setting_parameterValue_Svel[seqId]);
  } else if (setting_parameterIndex == 8) {
    // SETTING_PARAMETER_NAME_SNTE - sequence MIDI note
    strcpy(returnValue, SETTING_PARAMETER_VALUE_MIDINOTES[setting_parameterValue_Snte[seqId] - SETTING_PARAMETER_VALUE_SNTE_MIN]);
  } else if (setting_parameterIndex == 9) {
    // SETTING_PARAMETER_NAME_SRDA - sequence randomization amount
...
void setting_changeParameterValue(int32_t delta) {
  if (setting_parameterIndex == 0) {
    ...
    // SETTING_PARAMETER_NAME_SGTE - sequence gate length
    setting_parameterValue_tmp = min(SETTING_PARAMETER_VALUE_SGTE_MAX, max(SETTING_PARAMETER_VALUE_SGTE_MIN, setting_parameterValue_tmp + delta));
  } else if (setting_parameterIndex == 7) {
    // SETTING_PARAMETER_NAME_SVEL - sequence note velocity
    setting_parameterValue_tmp = min(SETTING_PARAMETER_VALUE_SVEL_MAX, max(SETTING_PARAMETER_VALUE_SVEL_MIN, setting_parameterValue_tmp + delta));
  } else if (setting_parameterIndex == 8) {
    // SETTING_PARAMETER_NAME_SNTE - sequence MIDI note
    setting_parameterValue_tmp = min(SETTING_PARAMETER_VALUE_SNTE_MAX - SETTING_PARAMETER_VALUE_SNTE_MIN, max(0, setting_parameterValue_tmp + delta));
  } else if (setting_parameterIndex == 9) {
    // SETTING_PARAMETER_NAME_SRDA - sequence randomization amount
